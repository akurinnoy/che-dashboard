...
import { selectBackups } from '@/store/Backups/selectors';
...
export class WorkspacesListContainer extends React.PureComponent<Props> {
  ...
  componentDidUpdate(prevProps: Props) {
    const prevUIDs = prevProps.allWorkspaces.map(w => w.uid).sort().join(',');
    const currUIDs = this.props.allWorkspaces.map(w => w.uid).sort().join(',');
...
const mapStateToProps = (state: RootState) => {
  return {
    ...
    backupsByWorkspace: selectBackups(state).byWorkspace,
