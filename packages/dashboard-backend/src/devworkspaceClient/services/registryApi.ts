...
export class RegistryApiService {
  ...
  async listBackupImages(
    ...
    page: number = 1,
    pageSize: number = BACKUP_LIST_DEFAULT_PAGE_SIZE,
    ...
    try {
      const timeoutPromise = new Promise<never>((_, reject) => {
        ...
        }, REGISTRY_QUERY_TIMEOUT_SECONDS * 1000);
      });
      ...
      this.setCache(cacheKey, enrichedBackups, BACKUP_CACHE_TTL_SECONDS);
  ...
  async validateBackupImage(imageUrl: string): Promise<BackupValidationResult> {
    ...
    try {
      ...
      if (result.valid && result.accessible) {
        this.setCache(cacheKey, result, BACKUP_CACHE_TTL_SECONDS);
      }
  ...
  async getImageMetadata(imageUrl: string): Promise<BackupItem> {
    ...
    try {
      ...
      // Cache the result
      this.setCache(cacheKey, backupItem, BACKUP_CACHE_TTL_SECONDS);
