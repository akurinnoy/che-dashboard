...
export class BackupApiService {
  ...
  /**
   * Discover PVC claim name from DevWorkspace
   * Generated by Claude Sonnet 4.5
   */
  private async getWorkspacePVCName(namespace: string, workspaceName: string): Promise<string> {
    try {
      ...
      const devworkspace = response as any;
      const pvcName =
        devworkspace.body?.status?.devworkspaceId ||
        devworkspace.body?.spec?.template?.attributes?.['controller.devfile.io/storage-type'];

      // Fallback to default pattern if not found
      return pvcName || `claim-${workspaceName}`;
    } catch (e) {
      // If workspace not found or error, use fallback pattern
      return `claim-${workspaceName}`;
    }
  }
  ...
  async triggerBackup(namespace: string, workspaceName: string): Promise<V1Job> {
    try {
      ...
      // Get configuration from operator config
      const response = await this.customObjectAPI.getClusterCustomObject({
        group: DEVWORKSPACE_OPERATOR_CONFIG_GROUP,
        version: DEVWORKSPACE_OPERATOR_CONFIG_VERSION,
        plural: DEVWORKSPACE_OPERATOR_CONFIG_PLURAL,
      ...
      const operatorConfig = response as unknown as { body: DevWorkspaceOperatorConfig };
      const backupJobConfig = operatorConfig.body.config?.workspace?.backupCronJob;

      // Validate required configuration
      const backupImage = backupJobConfig?.image;
      if (!backupImage) {
        throw new Error('Backup container image not configured in DevWorkspaceOperatorConfig');
      }

      const serviceAccount = backupJobConfig?.serviceAccountName || 'che-workspace';

      // Discover actual PVC name from DevWorkspace
      const pvcName = await this.getWorkspacePVCName(namespace, workspaceName);
      ...
      const response2 = await this.batchV1API.createNamespacedJob({
        namespace,
        body: jobManifest,
      });

      return (response2 as { body: V1Job }).body;
    } catch (e) {
      throw createError(
        e,
        BACKUP_JOB_ERROR_LABEL,
