...
export async function restoreWorkspace(
  ...
  try {
    const response = await AxiosWrapper.createToRetryMissedBearerTokenError().post(
      `${dashboardBackendPrefix}/namespace/${namespace}/devworkspaces`,
      { devworkspace: dw, restoreFromBackup: true, backupImageUrl },
    );
    return { devWorkspace: response.data, headers: response.headers };
  } catch (e) {
    const errorMessage = helpers.errors.getMessage(e);
    throw new Error(`Failed to restore workspace '${workspaceName}'. ${errorMessage}`);
  }
}
